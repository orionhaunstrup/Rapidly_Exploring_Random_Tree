...
## Recent fixes:
##
## I found the error that was making the start and end points
## occasionally appear inside of an obstacle
##
## Found and fixed the error that was printing out the error
## message and making the color default to white
...
class Simulation:
    ...
    def display(self, paint_demo = False):
        ...
        plt.yticks([])
        plt.gcf()
        plt.savefig(str(self.filename_counter) + ".png")
        self.filename_counter += 1
        plt.show()
        plt.close()
    ...
    def get_explorer_paint(self):
        ...
        choice = input('Enter "y" to accept or "n" to redo: ')
        if choice == "n":
            return self.get_explorer_paint()
        else:
            return paint
